...
def get_kibana_nodes_matrix(kibana_file,bins, predictors):
	#df = pd.read_csv(base + kibana_file)
	rm_quote = lambda x: x.replace('"', '')
	df = pd.read_csv(base + kibana_file, thousands=',')#, nrows = 3000)
	df = df.fillna(0)
...
def get_ordered_features_and_adjM_matrices(bins, predictors):
	A_pd = get_wiki_adj_matrix(wiki_file)
	kibana_nodes, kibana_matrix = get_kibana_nodes_matrix(kibana_file, bins, predictors)
...
def get_feat_prediction_cols(subject, predictors, col, kibana_matrix):
	...
	if subject == 'polarity_wo_hashtags':
		...
		rm_cols = [col for col in feat_cols if subject in col]
		print(rm_cols)
		if len(rm_cols)>0:
