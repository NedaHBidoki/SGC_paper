...
kibana_file = 'attr_74074.csv'#'user_attr_matrix2.csv', 'twitter_cve_topic_attr.csv'
...
def get_kibana_nodes_matrix(kibana_file,bins, pr_col, predictors):
	...
	df = pd.read_csv(base + kibana_file, thousands=',', nrows = 2000)
	df = df.fillna(0)
	df.columns = df.columns.str.replace(": Descending", "")
	df  = df.set_index('id_h')
	#print(df['extension.polarity'].min())
...
def get_ordered_features_and_adjM_matrices(bins, pr_col, predictors):
	A_pd = get_wiki_adj_matrix(wiki_file)
	kibana_nodes, kibana_matrix = get_kibana_nodes_matrix(kibana_file, bins, pr_col, predictors)
...
def get_feat_prediction_cols(subject, predictors, col, kibana_matrix):
	cols = list(kibana_matrix.columns)
	if predictors == 'topics':
		keyword = 'topic'
	elif predictors == 'hashtags':
	...
	if subject == 'polarity_wo_hashtags':
		...
		feat_cols = [col for col in feat_cols if not subject in col]
		#print(rm_cols)
		#if len(rm_cols)>0:
	...
	feat_cols = utils.feature_analysis(kibana_matrix[feat_cols+[pr_cols]],pr_cols)
	return feat_cols, pr_cols


def get_features_adjM_data(kibana_matrix,A,subject,predictors, col):
	...
	if subject == 'polarity_wo_hashtags' or subject == 'polarity_w_hashtags':
...
kibana_matrix,A = get_ordered_features_and_adjM_matrices([-1,0,1], 'polarity_category', 'hashtags')
