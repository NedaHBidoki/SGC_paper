...
def get_kibana_nodes_matrix(kibana_file,bins, pr_col, predictors):
	...
	mean_cols =[ 'extension.polarity', 'user.followers_count', 'extension.subjectivity', 'possibly_sensitive', 'user.friends_count']
	dictOffuncs = { i : 'count' for i in dum_df.columns if not i in mean_cols}
	dictOfmax ={ i : 'mean' for i in mean_cols}
	...
	dum_df['polarity_category'] = (dum_df['extension.polarity'] != 0).astype(int)
	dum_df['polarity_category']= pd.to_numeric(dum_df['polarity_category'])
	#print(df.loc[df['polarity_category']==1][['polarity_category','extension.polarity','possibly_sensitive']])
	dum_df = dum_df.drop('extension.polarity', axis=1)
	dum_df['possibly_sensitive'] = (dum_df['possibly_sensitive']>0).astype(int)
	dum_df['extension.subjectivity'] = (dum_df['extension.subjectivity']>0).astype(int)
	subject_cols = ['polarity_category', 'possibly_sensitive', 'extension.subjectivity']



	dum_df['polarity_category'].plot(kind='hist')
	plt.savefig('extension_polarity_after.png')
	plt.close()
	dum_df['possibly_sensitive'].plot(kind='hist')
	plt.savefig('possibly_sensitive_after.png')
	plt.close()
	dum_df['extension.subjectivity'].plot(kind='hist')
	plt.savefig('subjectivity_after.png')
	plt.close()
...
def visualize(G,A,wiki_file, attrM, label):
	attrs = attrM.to_dict('index')
	#print(attrs)
	nx.set_node_attributes(G, attrs)
	#print(G.nodes['8TkCte3dyYe_A7qGmLO-zw'])
	pos=nx.spring_layout(G)
	nx.draw(G,pos)
	nx.draw_networkx_nodes(G, pos, nodelist=A.index, node_color='r')
	plt.savefig('%s.png'%wiki_file.split('/')[-1])
	plt.close()
	plt.hist(attrM[label].tolist(), bins='auto')
	plt.ylabel('Count')
	plt.xlabel(label)	
	plt.title("%s in community %s with size %s"%(label, wiki_file.split('/')[-1].split('.')[0], len(attrM)))
	plt.grid()
	plt.savefig('%s_%s.png'%(label,wiki_file.split('/')[-1]))
...
def get_ordered_features_and_adjM_matrices(bins, col , subject, predictors, top, wiki_file):
	A_pd, G = get_wiki_adj_matrix(wiki_file)
	...
	#visualize(G,A_pd,wiki_file,kibana_matrix,pr_cols)
...
def get_feat_prediction_cols(subject, predictors, col, kibana_matrix, top):
	...
	elif subject == 'subjectivity':
		print('subject is subjectivity...')
		pr_cols= 'extension.subjectivity'
		feat_cols = [col for col in feat_cols if not subject in col] + ['user.followers_count', 'user.friends_count']
		#print(rm_cols)
		#if len(rm_cols)>0:
...
def get_features_adjM_data(kibana_matrix,A,subject,feat_cols, pr_cols):
	...
	if subject == 'polarity_wo_hashtags' or subject == 'polarity_w_hashtags' or subject == 'sensitivity' or subject == 'sensitivity':
