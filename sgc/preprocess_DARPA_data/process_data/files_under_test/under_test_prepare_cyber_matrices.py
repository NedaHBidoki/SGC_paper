...
wiki_file = 'network_%s.csv'%db
kibana_file = 'tw-cyber*-tweets.csv'#'user_attr_matrix2.csv'
...
def get_kibana_nodes_matrix(kibana_file):
	...
	df['hashtags'] = df['hashtags'].str.replace('hashtags_', '')
	df['hashtags'] = [x.strip('[]').split(',') for x in df['hashtags']]
	#df = df[df['id_h'].isin(nodes)]
	...
	if db == 'twitter_cyber':
		column_names_for_onehot = ["hashtags","possibly_sensitive"]
		df = df[column_names_for_onehot]
	df = pd.DataFrame(df.hashtags.dropna().tolist(),index=df.index).stack()
	df = df.reset_index([0, 'id_h'])
	df = df.rename(columns={0: "hashtags"})
	df = df.dropna()
	df['hashtags'] = [str(x) for x in df['hashtags']]
	df = df[df['hashtags']!= '']
	df['hashtags'] = 'hashtag_' + df['hashtags'].astype(str)
...
def get_wiki_adj_matrix(wiki_file, kibana_nodes):
	df = pd.read_csv(base + wiki_file)
	df = df[(df['node'].isin(kibana_nodes)) & (df['parent'].isin(kibana_nodes))]
	nodes = df['node'].values.tolist()
	parents = df['parent'].values.tolist()
...
def get_top_hashtags(kibana_matrix,n):
	...
	top_hashtags.to_csv('../predicted_hashtags/%s/top_hashtags/%s_largest_hashtags.csv'%(db,n))
...
def get_ordered_features_and_adjM_matrices():
	print('Getting feature and adj matrices...')
	kibana_nodes, kibana_matrix = get_kibana_nodes_matrix(kibana_file)
	A_pd, all_nodes = get_wiki_adj_matrix(wiki_file,kibana_nodes)
	print('A_pd info:',A_pd.info(memory_usage='deep'))
...
def customized_train_validate_test_split(pr_col, df, train_percent=.6, validate_percent=.2, seed=None):
    print('spliting data to train/validate/test sets...')
    #colname = sys.argv[1]
    colname = pr_col
...
def get_feat_prediction_cols(pr_col, kibana_matrix):
	cols = list(kibana_matrix.columns)
	keyword = 'hashtags'
	feat_cols = [c for c in cols if (keyword in c) and ('nan' not in c)]
	if pr_col == 'polarity':
		pr_cols = 'extension.polarity'
	elif pr_col == 'sensitivity':
	...
	#	sys.exit()
		...
		with open("../predicted_hashtags/%s/predicted hashtags.csv"%db, "a") as myfile:
...
def get_features_adjM_data(kibana_matrix,A,pr_col):
	...
	feat_cols, pr_cols = get_feat_prediction_cols(pr_col,kibana_matrix)
...
top = 50
kibana_matrix,A = get_ordered_features_and_adjM_matrices()
top_hashtags = get_top_hashtags(kibana_matrix,top)
print('kibana_matrix info:',kibana_matrix.info(memory_usage='deep'))
sys.exit()

pr_cols = get_features_adjM_data(kibana_matrix,A,top_hashtags[3])
