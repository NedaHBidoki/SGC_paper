...
def load_citation(dataset_str="cora", normalization="AugNormAdj", cuda=True):
    ...
    print('len(x)',np.shape(x),x)
    print('len(y)',np.shape(y),y)
    print('len(tx)',np.shape(tx),tx)
    print('len(ty)',np.shape(ty),ty)
    print('len(allx)',np.shape(allx),allx)
    print('len(ally)',np.shape(ally),ally)
